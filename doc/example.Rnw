...
\begin{document}
\def \KnitrHomePath {C:\GitHub\latex-knitr\doc}  %%added by BROCK, not sure if it is necessary
...
## then use the load() function to load the binary .RData environment. This
...
\rfoot{Appendix A -- Species Results}
