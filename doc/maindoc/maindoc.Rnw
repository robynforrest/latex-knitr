...
Before building this document, you need to install a couple of things which allow for the rendering of \emph{knitr} code in the document for example purposes. Open Rgui and do the following:
...
<<appendix-1, child='appendix-1/appendix-1.Rnw', eval=FALSE, literal=TRUE>>=
